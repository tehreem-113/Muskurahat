<<<<<<< HEAD
# Muskaan
A BDS third year project that aims to develop a website that helps raise dental health awareness and rvides easy access tools to book appointments in local clinics nearby.
=======
# Muskaan (مسکان) — Digital Oral Health Platform Prototype

A clickable, front-end prototype for **Muskaan**, a digital oral health platform
...
`muskaan/` folder (this shows both `frontend/` and `backend/` side by side).
...
>>>>>>> 101c710 (Initial commit of Code Files)
